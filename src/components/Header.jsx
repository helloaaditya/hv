    ...
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className={`lg:hidden transition-all duration-500 ease-in-out ${
          ...
          <div className="px-4 py-6 space-y-2 flex flex-col gap-2">
            {navItems.map((item, index) => (
              <a
                key={item.name}
                href={item.path}
                onClick={() => handleNavClick(item.name)}
                className={`block w-full px-5 py-4 rounded-xl font-medium text-base transition-all duration-300 transform hover:scale-105 ${
                  activeItem === item.name
            ...
            <a
              href="/quote"
              onClick={closeMenu}
              className="block w-full mt-4 px-5 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl text-center hover:from-blue-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
            ...
            {/* Mobile Contact Info */}
            <div className="mt-6 pt-6 border-t border-gray-200/50 flex flex-col gap-3">
